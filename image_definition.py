...
def save_to_bd():
    ...
    2. Когда находим 1 - запускаем проверку на наличие рядом точки == 1 по следующему алгоритму:
     2.1. Проверить слева; затем сверху справа (северо-восток);  сверху; слева сверху (северо-запад);
        2.2. Если горящая точка не находится ни в одном из этих направлений - то текущая точка - связывается с точкой 
            (start), назначить posl_tg = ID (start). Posl_tg - это последняя горящая точка (tg), для упрощения поиска
        2.3. Если была найдена точка == 1 слева, то:
            2.3.1. Найти связь между этой координатой и (tg)
            2.3.3. Проверить имеется ли между (east) и Posl_tg точка (tg), которая их связывает (для этого в таблице 
            точек нужно обязательно указывать rod1 и rod2, где rod1 - это точка направления east, south и т.п., а rod2 - 
            это (tg)).
            2.3.4. Если да
                    2.3.4.1. Присвоить этой найденной tg = posl_tg
            2.3.5. Если неn
                    2.3.5.1. Создать новую временную точку (tg)
                    2.3.5.2. Создать связь между posl_tg и новой (tg) и между east и новой (tg)
                    2.3.5.3. Присвоить posl_tg = новой tg
            2.3.6. Создать связь между текущей точкой сетки и posl_tg
            2.3.7. Создать связь где начало - это найденная точка координат, а конец - текущая точка
        2.4. Если была найдена точка == 1 в другой координате кроме левой:
            2.3.1. Проверить имеется ли у найденной точки связь с другой точкой сетки, где начало стрелки - это 
                    найденная точка, так найти последнюю точку в цепочке
            2.3.2. Найти связь с tg у этой последней точки сетки
            2.3.3. Присвоить posl_tg этой точке tg
            2.3.4. Создать связь с найденной последней точкой сетки и текущей точкой сетки (направление
                   от последней к текущей)
            2.3.5. Проверить имеется ли (tg) между posl_tg и текущей точкой направления
                    2.3.6.1. Если да - то зажечь эту (tg) и присвоить ей posl_tg
                    2.3.6.2. Если нет - то создать новую точку (tg), соединив posl_tg и горящую точку направления,
